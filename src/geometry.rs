...
pub fn point_drawer() {
    ...
    'event : loop {
        ...
        renderer.set_draw_color(if bright { white } else { black });
        frame += 1;
        if frame % 60 == 0{
            bright = !bright;
        }
        ...
        let rect = Rect::new(SCREEN_WIDTH as i32/4, SCREEN_HEIGHT as i32/4,
                             SCREEN_WIDTH/2, SCREEN_HEIGHT/2).ok().expect("sdl create rect failed")
                                                             .expect("width or height must not be 0");
        renderer.set_draw_color(red);
        renderer.fill_rect(rect);

        let unfilled_rect = Rect::new(SCREEN_WIDTH as i32/6, SCREEN_HEIGHT as i32/6,
